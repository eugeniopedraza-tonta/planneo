...
export default function GuidesSection() {
  return (
    <section id="diario" className="bg-[#0E0B1A] px-4 py-20 text-[#F5F0FF] sm:px-6 lg:px-14 lg:py-28">
      <div className="mx-auto max-w-7xl">
        <p className="v4-mono mb-4 text-[11px] text-[#C77DFF]">{"// DIARIO"}</p>
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {guides.map(([meta, title, excerpt]) => (
            <article key={title} className="overflow-hidden rounded-[24px] border border-white/10 bg-white/[0.04]">
              ...
              <div className="p-6">
                <p className="v4-mono text-[10px] text-[#C77DFF]">{meta}</p>
                ...
                <span className="mt-6 inline-block text-sm font-semibold text-[#C77DFF]">Leer guía →</span>
              </div>
            </article>
          ))}
        </div>
