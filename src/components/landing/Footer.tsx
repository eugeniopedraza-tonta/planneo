...
export default function Footer() {
  return (
    <footer className="border-t border-white/10 bg-[#0E0B1A] px-4 py-14 text-[#F5F0FF] sm:px-6 lg:px-14">
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.1fr_1fr_360px]">
        ...
        <div className="grid grid-cols-3 gap-6">
          {Object.entries(FOOTER_LINKS).map(([category, links]) => (
            <div key={category}>
              <h3 className="v4-mono mb-4 text-[10px] text-[#C77DFF]">{category}</h3>
        ...
        <div className="rounded-[24px] border border-white/10 bg-white/[0.04] p-6">
          <p className="v4-mono text-[10px] text-[#06D6A0]">¿ERES PROVEEDOR?</p>
          ...
          <Link href="/login" className="mt-5 inline-flex min-h-11 items-center justify-center rounded-xl bg-[#7B2CBF] px-5 text-sm font-semibold text-white transition hover:bg-[#6B22AE]">
            Ir al panel
          </Link>
        </div>
      </div>

      <div className="mx-auto mt-12 flex max-w-7xl flex-col gap-3 border-t border-white/10 pt-6 text-xs text-white/40 sm:flex-row sm:items-center sm:justify-between">
