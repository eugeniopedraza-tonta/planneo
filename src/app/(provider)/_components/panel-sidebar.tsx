  ...
  return (
    <aside className="w-56 shrink-0 bg-white border-r border-gray-200 flex flex-col">
      <div className="h-16 flex items-center px-5 border-b border-gray-200">
        <Link href="/panel" className="text-lg font-bold text-[#7C3AED]">
          Planneo
        </Link>
      ...
      {provider && (
        <div className="px-4 py-3 border-b border-gray-100">
          <p className="text-xs font-medium text-gray-900 truncate">{provider.name}</p>
          <p className="text-xs text-gray-400 truncate capitalize">{provider.categories?.slug ?? 'Sin categoría'}</p>
        </div>
      )}

      {role === 'provider_pending' && (
        <div className="mx-3 mt-3 rounded-xl bg-amber-50 border border-amber-200 px-3 py-2">
          <p className="text-xs text-amber-700 font-medium">Perfil en revisión</p>
          <p className="text-xs text-amber-600 mt-0.5">Aprobamos en 24–48 hrs.</p>
        </div>
      )}
      ...
      <div className="p-3 border-t border-gray-200 flex flex-col gap-1">
        ...
        <button
          ...
          className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-colors w-full text-left"
        >
          <LogoutIcon />
          Cerrar sesión
        </button>
...
function DashboardIcon({ active }: { active?: boolean }) {
  return (
    <svg className={`size-4 shrink-0 ${active ? 'text-[#7C3AED]' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function EyeIcon() {
  return (
    <svg className="size-4 shrink-0 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
